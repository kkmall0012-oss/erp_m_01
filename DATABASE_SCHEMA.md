...
3. [資料表 1：`company_profile`（公司基本主檔與表單套印）](#三資料表-1company_profile公司基本主檔與表單套印)
4. [資料表 2：`transactions`（零用金核心收支流水帳）](#四資料表-2transactions-零用金核心收支流水帳)
5. [資料表 3：`categories`（多階層分類與自訂選單）](#五資料表-3categories多階層分類與自訂選單)
6. [資料表 4：`claimants`（經辦同仁與請領人名冊）](#六資料表-4claimants經辦同仁與請領人名冊)
7. [資料表 5：`budgets`（月份預算與安全水位設定）](#七資料表-5budgets月份預算與安全水位設定)
8. [資料表 6：`sub_accounts`（專案採買專款子帳戶）](#八資料表-6sub_accounts專案採買專款子帳戶)
9. [資料表 7：`director_withdrawals`（廠長/主管專用提領紀錄）](#九資料表-7director_withdrawals廠長主管專用提領紀錄)
10. [現成資料直接撈取指南（免重複設欄位之關鍵設計）](#十現成資料直接撈取指南免重複設欄位之關鍵設計)
11. [常用 SQL 快速查詢範例（隨查隨用）](#十一常用-sql-快速查詢範例隨查隨用)
...
└─────────────────────────────────────────────────────────────┘
          │
          ├─► [company_profile]       公司核心主檔（全名、統編、負責人、地址、銀行、表單抬頭）
          │
...
## 三、資料表 1：`company_profile`（公司基本主檔與表單套印）

### 1. 資料表簡介
作為全 ERP 系統最高層級的「公司組織主檔」，儲存公司全稱、簡稱、統一編號、負責人、登記地址、通訊電話、匯款銀行帳號、會計與出納簽核人名以及表單抬頭範本。未來系統內無論是「零用金報表列印」、「車輛油資核銷表單」、「資產領用單」或「請購簽核單」，皆自動自本資料表讀取最新資料套印，免人工重複鍵入。

### 2. 欄位結構定義表

| 欄位名稱 (Column) | SQLite 型態 | 必填/預設 | 對應 TypeScript 型別 | 變數代表意思 (商業含義) | 系統使用的地方與業務邏輯 | 未來程式直接撈取指引 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **`id`** | `TEXT` | **PRIMARY KEY** | `string` | 單一主檔識別碼 | 固定值 `'default'`，代表本組織之公司主要設定檔。 | `SELECT * FROM company_profile WHERE id = 'default'`。 |
| **`name`** | `TEXT` | NOT NULL | `string` | 公司正式全名 | 頂部導覽列抬頭、所有 A4 報表正上方表頭、外部正式請款單據。 | 例如：「宏達精密科技股份有限公司」。 |
| **`shortName`** | `TEXT` | NOT NULL | `string` | 公司簡稱 | 側邊欄折疊縮寫、報表簡稱、名牌與內部簡報。 | 例如：「宏達精密」。 |
| **`taxId`** | `TEXT` | `''` | `string` | 統一編號 (8碼統編) | 發票開立對照、扣抵憑單、銀行表單、採購單。 | 例如：`12345678`。 |
| **`representative`** | `TEXT` | `''` | `string` | 公司法定負責人/代表人 | 合約書、正式公文、最高層級簽章處。 | 例如：「陳董事長」。 |
| **`phone`** | `TEXT` | `''` | `string` | 公司代表號電話 | 報表聯絡資料、外部訂單表頭聯絡窗口。 | 例如：`02-2345-6789`。 |
| **`fax`** | `TEXT` | `''` | `string` | 傳真號碼 | 傳真採購確認單、詢價單頁尾。 | 例如：`02-2345-6790`。 |
| **`email`** | `TEXT` | `''` | `string` | 聯絡或財務專用 Email | 電子發票寄發、對帳聯絡信箱。 | 例如：`finance@example.com`。 |
| **`address`** | `TEXT` | `''` | `string` | 公司登記/聯絡營業地址 | 請款單寄送地址、送貨單發票發貨處。 | 例如：「新北市汐止區新台五路一段100號」。 |
| **`bankName`** | `TEXT` | `''` | `string` | 往來銀行名稱與分行 | 請款單據匯款指引、員工代墊款轉帳帳號。 | 例如：「台灣銀行 (004) 南港分行」。 |
| **`bankAccount`** | `TEXT` | `''` | `string` | 公司銀行匯款帳號 | 銀行對帳、廠商付款指定受款帳號。 | 例如：`012-345-678901`。 |
| **`bankAccountName`**| `TEXT` | `''` | `string` | 銀行帳戶戶名 | 匯款受款人比對，避免同仁或廠商匯錯戶名。 | 例如：「宏達精密科技股份有限公司」。 |
| **`currency`** | `TEXT` | `'TWD'` | `string` | 預設記帳幣別代碼 | 報表幣別註記、匯率換算預設值。 | 預設 `TWD` (新台幣)。 |
| **`taxInvoiceNote`** | `TEXT` | `''` | `string` | 發票開立與報帳特別備註 | 顯示於報表下方或報帳指引，例如「發票開立請務必打統編並開立二/三聯式」。 | 報表底部提醒專用文字。 |
| **`reportHeader`** | `TEXT` | `''` | `string` | 正式列印表頭主標題 | 自訂套表抬頭，為空時預設以「公司名稱 + 報表名稱」。 | 供各類報表自動套用。 |
| **`chiefAccountant`**| `TEXT` | `''` | `string` | 主辦會計/財務主管姓名 | 各項正式傳票、零用金報表簽核預設審核人。 | 報表審核簽核欄自動帶出。 |
| **`cashier`** | `TEXT` | `''` | `string` | 出納專員姓名 | 現金收支明細表、撥補申請單出納簽核人。 | 報表審核簽核欄自動帶出。 |
| **`updatedAt`** | `TEXT` | CURRENT_TIMESTAMP | `string` | 最後變更修改時間 | 稽核紀錄、資料庫最新時間戳記。 | ISO 日期時間字串。 |

---

## 四、資料表 2：`transactions`（零用金核心收支流水帳）
